...
func TelegramDefaultHandler(ctx context.Context, bot *tgbot.Bot, update *tgmodels.Update) {
	...
	if update.InlineQuery != nil {
		g.Log().Debugf(ctx, "Telegram Inline: [%d] -> [%s]",
...
func TelegramParseDuoCommand(ctx context.Context, cmd string) (urls []string, characters []string) {
	...
	case 4:
		b64Text := cmds[3]
		_, err := base64.RawURLEncoding.DecodeString(b64Text)
		if len(b64Text) <= 0 || err != nil {
			return []string{consts.MyDuoElements{
				Rounded:        true,
				Character:      consts.Lily,
				Language:       consts.English,
				OriginText:     "The command seems broken!",
				TranslatedText: "命令看起来是坏掉的！",
				ToJpeg:         true,
			}.ToUrl(serverBase)}, []string{consts.Lily.ToString()}
		}
		return []string{serverBase + "/b/" + b64Text}, []string{"(custom)"}
	default:
		return []string{consts.MyDuoElements{
			Rounded:        true,
			Character:      consts.Lily,
...
func TelegramSendDuoImage(ctx context.Context, bot *tgbot.Bot, req *tgmodels.InlineQuery, cmd string) {
	...
	for i, v := range r {
		...
		g.Log().Debugf(ctx, "Telegram Inline: [%s] img [%s]", req.ID, v)
	}
	...
	g.Log().Infof(ctx, "Telegram Inline: [%s] replying...", req.ID)
	_, err := bot.AnswerInlineQuery(ctx, &result)
	if err != nil {
		g.Log().Errorf(ctx, "Telegram Inline: [%s] reply failed: %s", req.ID, err.Error())
		return
	}
	g.Log().Infof(ctx, "Telegram Inline: [%s] replied", req.ID)
}
